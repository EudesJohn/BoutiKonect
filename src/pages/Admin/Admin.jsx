...
export default function Admin() {
  const { seller, user, getAllUsers, getAllProducts, services, getAllOrders, getReportedProducts, deleteUser, deleteReportedProduct, deleteService, resolveReport, getAllReports, messages, allUsers, formatPrice } = useContext(AppContext)
  ...
  const isAdminUser = (seller?.isAdmin || user?.isAdmin) || 
    (seller?.email === 'eudesjohn650@gmail.com') ||
    (user?.email === 'eudesjohn650@gmail.com') ||
    (seller?.email && isAdminEmail(seller.email)) || 
  ...
  const handleDeleteProduct = async (productId) => {
    ...
    if (confirmed) {
      deleteReportedProduct(productId)
                  ...
                  <div key={sellerId} className="seller-orders-group">
                    <div className="seller-orders-header">
                      <h3>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', fontSize: '12px', fontWeight: 'normal', color: 'var(--dark-text-tertiary)', marginTop: '4px' }}>
                        ...
                        <table className="orders-table">
                          ...
                          <tbody>
                            {orders.map(order => (
                              <tr key={order.id}>
                                <td data-label="Produit">
                                    <div style={{ color: order.type === 'service' ? 'var(--primary)' : 'var(--gold)', fontWeight: '700' }}>
                                ...
                                <td data-label="Prix/Total">
                                    ...
                                    <div style={{ color: 'var(--primary)', fontWeight: '700' }}>Total: {order.type === 'service' && order.priceType === 'Devis' ? 'Sur Devis' : formatPrice(order.total || order.price * order.quantity)}</div>
                                </td>
                                <td data-label="Client">
                                    <div style={{ fontWeight: '700' }}>{order.buyerName}</div>
                                    <div style={{ fontSize: '12px' }}><Phone size={10} /> {order.buyerPhone}</div>
                                    {order.type === 'service' ? (
                                      <div style={{ fontSize: '11px', color: 'var(--dark-text-tertiary)', marginTop: '4px', padding: '4px', background: 'rgba(255,106,0,0.05)', borderRadius: '4px' }}>
                                        <div>📍 {order.location}</div>
                                        <div style={{ whiteSpace: 'pre-wrap' }}>📝 {order.details}</div>
                                      </div>
                                    ) : (
                                      <div style={{ fontSize: '11px', color: 'var(--dark-text-tertiary)' }}>🏠 {order.buyerAddress}</div>
                                    )}
                                </td>
                                <td data-label="Date">
