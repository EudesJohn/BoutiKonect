      ...
      <div className="container">
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showShareMenu && (
                <div className="share-menu-dropdown" style={{ 
                  position: 'absolute', top: '100%', right: 0, background: 'var(--dark-bg-card)',
        ...
        <div className="service-detail-grid">
          ...
          <div className="service-info-section">
            <div className="service-header-info">
              ...
              <div className="service-rating-summary" style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', color: 'var(--gold)' }}>
                <Star fill={itemRating.count > 0 ? "var(--gold)" : "none"} size={20} />
        ...
        <div className="service-full-description" style={{ marginTop: '40px' }}>
          ...
          {itemReviews.length > 0 ? (
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <span style={{ fontWeight: '700', color: 'var(--dark-text-primary)' }}>{rev.reviewerName}</span>
                    ...
                    <div style={{ display: 'flex', color: 'var(--gold)' }}>{[...Array(5)].map((_, i) => <Star key={i} size={14} fill={i < rev.rating ? "var(--gold)" : "none"} />)}</div>
                  </div>
                  <p style={{ color: 'var(--dark-text-tertiary)', fontSize: '0.95rem' }}>{rev.comment}</p>
                </div>
              ))}
            ...
            <p className="no-reviews" style={{ color: 'var(--dark-text-tertiary)', fontStyle: 'italic', marginTop: '10px' }}>Aucun avis pour le moment.</p>
          )}
        </div>
        ...
        {showReviewModal && (
          <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="modal-content" style={{ background: 'var(--dark-bg-card)', padding: '30px', borderRadius: '20px', width: '100%', maxWidth: '500px', position: 'relative' }}>
              <button className="close-modal" onClick={() => setShowReviewModal(false)} style={{ position: 'absolute', top: '15px', right: '15px', background: 'none', border: 'none', color: 'var(--dark-text-tertiary)', cursor: 'pointer' }}><X size={24} /></button>
              <h2 style={{ color: 'var(--primary)', marginBottom: '20px' }}>Noter ce service</h2>
              {reviewSuccess ? (
                <div style={{ textAlign: 'center', padding: '20px' }}><Star size={48} color="var(--success)" style={{ marginBottom: '10px' }} /><p style={{ color: 'var(--success)', fontWeight: '600' }}>Merci pour votre avis !</p></div>
              ) : (
                <form onSubmit={handleAddReview}>
                  <div className="rating-select" style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginBottom: '25px' }}>
                    {[1, 2, 3, 4, 5].map(num => (
                      <button key={num} type="button" onClick={() => setNewReview({ ...newReview, rating: num })} style={{ background: 'none', border: 'none', cursor: 'pointer', color: num <= newReview.rating ? 'var(--gold)' : 'var(--dark-text-tertiary)' }}>
                        <Star size={32} fill={num <= newReview.rating ? 'var(--gold)' : 'none'} />
                      </button>
                    ))}
                  </div>
        ...
        {showReportModal && (
          <div className="modal-overlay" onClick={() => setShowReportModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
            <motion.div 
              ...
              style={{ background: 'var(--dark-bg-card)', padding: '30px', borderRadius: '24px', width: '100%', maxWidth: '500px', position: 'relative' }}
            >
              <button className="close-modal" onClick={() => setShowReportModal(false)} style={{ position: 'absolute', top: '20px', right: '20px', background: 'none', border: 'none', color: 'var(--dark-text-tertiary)' }}><X size={24} /></button>
              <h2 style={{ color: 'var(--danger)', marginBottom: '15px' }}><Flag size={24} style={{ marginRight: '10px' }} /> Signaler ce service</h2>
              <form onSubmit={async (e) => {
                ...
                <div className="form-group" style={{ marginBottom: '20px' }}>
                  <label style={{ display: 'block', marginBottom: '10px', color: 'var(--dark-text-tertiary)' }}>Motif du signalement</label>
                  <select 
                    value={reportReason} 
