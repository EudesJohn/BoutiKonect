...
export default function Login() {
  const { setUser, setSeller, loginUser, rememberMe, setRememberMe, resolveMFASignIn } = useContext(AppContext)
  ...
  const [mfaData, setMfaData] = useState({ resolver: null, hints: [] })
    ...
    return (
      <div className="login-page">
        ...
        <MFAModal 
          ...
          uid={mfaData.uid}
          email={mfaData.email}
          onVerify={async (userResult, code) => {
            if (mfaData.mfaType === 'email') {
              // Si c'est un e-mail, userResult contient déjà l'utilisateur
              setShowMFAModal(false)
              setSuccess(true)
              setTimeout(() => navigate('/'), 1000)
            } else {
              const result = await resolveMFASignIn(mfaData.resolver, null, code)
              if (result.success) {
                setShowMFAModal(false)
                setSuccess(true)
                setTimeout(() => navigate('/'), 1000)
