...
export default function SellerProfile() {
  ...
  const totalRevenue = sellerOrders
    ...
    .reduce((sum, o) => sum + (o.total || o.price * o.quantity || 0), 0)
  ...
  const getLast7DaysRevenue = () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayOrders = sellerOrders.filter(o => 
        o.createdAt && o.createdAt.startsWith(dateStr) && o.paymentStatus === 'paid'
      )
      const dayRevenue = dayOrders.reduce((sum, o) => sum + (o.total || o.price * o.quantity || 0), 0)
...
<td>{formatPrice(order.total || order.price * order.quantity)}</td>
                          <td>
        ...
        >
          ...
          {sellerProducts.length > 0 ? (
            ...
            <div className="no-products" style={{ marginBottom: '50px', textAlign: 'center', padding: '40px', background: 'rgba(255,255,255,0.03)', borderRadius: '15px' }}>
              <Package size={48} color="var(--dark-text-tertiary)" />
              <p style={{ color: 'var(--dark-text-tertiary)', marginTop: '10px' }}>Ce vendeur n'a pas encore de produits</p>
            </div>
          )}
          ...
          {sellerServices.length > 0 ? (
            ...
            <div className="no-products" style={{ textAlign: 'center', padding: '40px', background: 'rgba(255,255,255,0.03)', borderRadius: '15px' }}>
              <Briefcase size={48} color="var(--dark-text-tertiary)" />
              <p style={{ color: 'var(--dark-text-tertiary)', marginTop: '10px' }}>Ce vendeur n'a pas encore de services</p>
            </div>
          )}
        </motion.div>
      </div>
