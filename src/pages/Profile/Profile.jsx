...
import { AppContext, cities } from '../../context/AppContext'
...
export default function Profile() {
  const { user, seller, logoutUser, products, services, getFavoriteProducts, getFavoriteServices, toggleFavorite, isFavorite, getSellerOrders, updateOrderStatus, updateProfile, upgradeToSeller, orders, formatPrice, updateEmailWithVerification, resetPassword } = useContext(AppContext)
  const navigate = useNavigate()
  const [activeTab, setActiveTab] = useState('favorites')
  const [isEditing, setIsEditing] = useState(false)
  const [showConfirmModal, setShowConfirmModal] = useState(false)
  const [editForm, setEditForm] = useState({ name: '', email: '', phone: '', city: '', neighborhood: '', avatar: '', newPassword: '' })
  ...
  const sellerStats = seller ? {
    totalRevenue: sellerOrders.filter(o => o.status === 'completed' || o.status === 'confirmed').reduce((sum, o) => sum + (o.total || o.price * o.quantity), 0),
  ...
  const getOrdersByDay = () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const revenue = dayOrders.filter(o => o.status === 'completed' || o.status === 'confirmed').reduce((sum, o) => sum + (o.total || o.price * o.quantity), 0)
  ...
  const handleEditClick = () => {
    ...
    setEditForm({ name: currentUser?.name || '', email: currentUser?.email || '', phone: currentUser?.phone || '', city: currentUser?.city || '', neighborhood: currentUser?.neighborhood || '', avatar: currentUser?.avatar || '', newPassword: '' })
    setIsEditing(true)
  }
  ...
  const confirmSaveProfile = async () => {
    ...
    try {
      // Update simple fields in firestore
      ...
      // Handle Email update via Firebase Auth if it changed
      if (editForm.email !== currentUser.email && !currentUser.isGoogle) {
        const result = await updateEmailWithVerification(editForm.email);
        if (result.success) {
          alert("Un e-mail de confirmation a été envoyé à " + editForm.email + ". Le changement sera effectif après validation.");
        } else {
          alert(result.error || "Erreur lors du changement d'email.");
        }
      }
      ...
      <div className="container">
        <motion.div className="profile-header" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="profile-actions">
            {!isEditing && (
              <>
                {!seller && <button className="btn btn-primary" onClick={upgradeToSeller}><Store size={18} />Devenir vendeur</button>}
                ...
                {seller ? (
                  <>
                    <div className="stat-card" style={{ background: 'linear-gradient(135deg, #4CAF50, #2E7D32)', color: 'white' }}>
                      <DollarSign size={24} />
                      <span className="stat-number" style={{ fontSize: '1.3rem' }}>{formatPrice(sellerStats?.totalRevenue || 0)}</span>
