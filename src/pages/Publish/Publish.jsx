...
import { AppContext, categories, serviceCategories } from '../../context/AppContext'
...
export default function Publish() {
  ...
  const sellerProducts = isAdmin ? products : products.filter(p => p.sellerId === seller?.id)
  const sellerServices = isAdmin ? services : services.filter(s => s.sellerId === seller?.id)
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      if (publishType === 'product') {
        ...
        delete itemData.priceType
        delete itemData.experience
        delete itemData.duration
      } else {
        itemData.priceType = formData.priceType || 'Fixe'
        itemData.experience = formData.experience
        itemData.duration = formData.duration
        // Les services sont toujours disponibles, pas de notion de stock
