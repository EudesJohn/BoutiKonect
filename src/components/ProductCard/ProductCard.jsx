      ...
      <div className="product-card">
            ...
            <div className="product-badges">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                {product.type !== 'service' && product.condition && (
                  <span className="badge" style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', color: 'white', fontSize: '10px', padding: '2px 8px', borderRadius: '4px' }}>
                    {product.condition}
                  </span>
                )}
              </div>
        ...
        <div className="product-content">
          ...
          <div className="product-price">
            {product.type === 'service' ? (
              product.priceType === 'Devis' || !product.price ? (
                <span className="price-devis" style={{ color: 'var(--secondary)' }}>Sur Devis</span>
              ) : (
                <>
                  <span className="price-label">À partir de </span>
                  <span className="price-amount">{formatPrice(product.price)}</span>
                  {product.priceType && product.priceType !== 'Fixe' && (
                    <span className="price-unit" style={{ fontSize: '0.9rem', color: 'var(--dark-text-tertiary)', marginLeft: '2px' }}>
          ...
          <div className="product-actions">
            ...
            >
              {product.type === 'service' ? <Zap size={16} /> : <ShoppingCart size={16} />}
              {product.type === 'service' ? 'Réserver' : (product.stock > 0 ? 'Ajouter' : 'Indisponible')}
            </button>
            <a 
              href={whatsappUrl}
              target="_blank"
